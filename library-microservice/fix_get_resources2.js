...
for (let i=0; i<lines.length; i++) {
  if (lines[i].includes('app.get("/resources", async (req, res) => {')) {
    start = i;
  }
...
lines.splice(start, end - start + 1, newHandler);
fs.writeFileSync('src/index.js', lines.join('\n'));
console.log("Successfully replaced block from line", start, "to", end);
