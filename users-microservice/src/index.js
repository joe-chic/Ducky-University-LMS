...
app.post("/auth/login", async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT u.user_id, u.campus_id, u.user_email, u.user_phone, u.user_state, u.user_password,
...
// GET /users/:campus_id/by-campus — buscar usuario por campus_id
app.get("/users/:campus_id/by-campus", async (req, res) => {
  try {
    const campus_id = Number(req.params.campus_id);
    if (!campus_id) return res.status(400).json({ message: "campus_id invalido" });

    const result = await query(
      `SELECT u.user_id, u.campus_id, u.first_name, u.father_lastname,
              u.user_email, u.user_state, r.role_name
       FROM users u
       JOIN roles r ON r.role_id = u.role_id
       WHERE u.campus_id = $1
       LIMIT 1`,
    ...
    res.json({
      ...
      nombre: `${u.first_name} ${u.father_lastname}`,
      email: u.user_email,
